...
import { QUERY_GET_TOP_QUESTION_BY_PARTICIPANTS } from '@/service/graphql/Forum';
...
const HealthSearchInterfaceOne: React.FC = () => {
  // --- State ---
  ...
  // --- Infinite Categories Picker State ---
  ...
  const { data: topQuestionData } = useQuery<TopQuestionsData>(
    QUERY_GET_TOP_QUESTION_BY_PARTICIPANTS,
  );
  const { data: recentBlogData } = useQuery<RecentBlogData>(QUERY_RECENT_BLOG, {
    client: cmsClient,
  });
  const { data: recentVideoData } = useQuery<RecentVideoData>(
    QUERY_RECENT_VIDEO,
    {
      client: cmsClient,
    },
  ...
  useEffect(() => {
    ...
    const midBase = Math.floor(REPEATS / 2) * n;
    const startAbs = midBase + 0;

    setIsIntroAnimating(true);
    setCategoryAbsIndex(startAbs);

    requestAnimationFrame(() => {
      scrollToAbsIndexCentered(startAbs, 'auto');
      ...
      const timer = window.setInterval(() => {
        ...
        if (step >= n) {
          ...
          const endAbs = midBase + (n - 1);
          setCategoryAbsIndex(endAbs);

          requestAnimationFrame(() => {
            scrollToAbsIndexCentered(endAbs, 'smooth');
            setTimeout(() => setIsIntroAnimating(false), 250);
          });

          return;
        }

        const nextAbs = midBase + step;
        setCategoryAbsIndex(nextAbs);
        scrollToAbsIndexCentered(nextAbs, 'smooth');
      }, 120);
  ...
  const contentCards = [
    {
      type: 'Question',
      title:
        topQuestionData?.questions?.[0]?.title ||
        'Will I able to walk with ACL injury?',
      icon: ExternalLink,
      badge: 'Question',
      cols: 1,
    },
    {
      type: 'Blog',
      title:
        recentBlogData?.sunriseBlogs?.data[0]?.attributes?.Title ||
        'Why Smoking a Cigarette Before or After Your Workout Can Harm Your ...',
      icon: ExternalLink,
      badge: 'Blog',
      cols: 1,
    },
    {
      type: 'Video',
      title:
        recentVideoData?.youtubes?.data[0]?.attributes?.Title ||
        'Loneliness: A Silent Killer - Video by Kofuku',
      icon: PlayCircle,
      badge: 'Video',
      cols: 1,
    },
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-[#050505] text-white font-sans flex overflow-hidden">
              ...
              {repeatedAbsIndices.map((absIdx) => {
                ...
                return (
                  <Link
                    ...
                    href={`/salus/${category?.attributes?.slug}`}
                    style={getItemStyle(distance)}
                    className={`w-[250px] mx-auto h-[52px] flex items-center gap-4 pl-4 pr-6 rounded-full border transition-all duration-300
                      ${isInCenter
                        ? 'bg-white/25 border-white/10'
                        : isAdjacent
                          ? 'bg-white/25 border-white/10 hover:bg-white/15 hover:border-white/15'
      ...
      <main className="flex-1 flex flex-col relative bg-transparent overflow-hidden z-10">
        <div className="flex-1 overflow-y-auto px-10 pb-2 relative z-10 custom-scrollbar">
          {chatOpen ? (
            ...
            <div className="max-w-5xl mx-auto pt-10">
              ...
              <div className="grid grid-cols-2 gap-6">
                {toolsCards.map((tool, idx) => (
                  <div
                    key={idx}
                    className="bg-[#18181b] rounded-3xl p-6 h-64 relative overflow-hidden group hover:bg-[#202024] transition-colors cursor-pointer border border-white/5"
                    style={
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      <h3 className="text-2xl font-normal text-white/90 mb-4">
                        {tool.title}
                      </h3>
                      {tool.content}
                    </div>
...
export default HealthSearchInterfaceOne;
