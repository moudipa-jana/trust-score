...
export default function TopReads({ topReads }: TopReadsProps) {
  const router = useRouter();
  const { categoryName } = router.query;
  const [selectedBlogSlug, setSelectedBlogSlug] = useState<string | null>(null);

  const topreadUrl = `${categoryName}/top-read`;

  const sortedBlogs = topReads
    .sort((a: Blog, b: Blog) => b.attributes.views - a.attributes.views);

  return (
    <div className="pickSection pt-10">
      <div className="pb-5">
        <Link href={`${topreadUrl}`}>
          <span className="dottedSectionTitle">Top reads</span>
        </Link>
      </div>
